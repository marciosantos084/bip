...
jQuery(document).ready(function ($) {
    ...
    $('.dismiss-button-recommended-plugin').click(function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $('#' + id).parent().append('<div id="temp_load" style="text-align:center"><img src="' + customizer_notice_data.base_path + '/img/spinner-2x.gif" /></div>');
            },
            success: function (data) {
                var container = $('#' + data).parent().parent();
                container.slideToggle().remove();
                
                if( $('.recomended-actions_container > .bip-recommended-plugins').length === 0 ){
