<?php
/**
 * bip Customizer Notification System.
 *
 * @package bip
 */
...
class bip_Customizer_Notice {
	...
	/**
	 * The single instance of bip_Customizer_Notice
	 *
	 * @var bip_Customizer_Notice $instance The bip_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main bip_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of bip_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof bip_Customizer_Notice ) ) {
			self::$instance = new bip_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $bip_recommended_plugins;
		...
		$bip_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$bip_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'bip-customizer-notice', get_template_directory_uri() . '/lib/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'bip-customizer-notice', get_template_directory_uri() . '/lib/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'bip-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'bip' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the bip_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'bip_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new bip_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'bip_show_recommended_plugins' );
			...
			update_option( 'bip_show_recommended_plugins', $show_recommended_plugins );
		}
