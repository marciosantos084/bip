<?php
/**
 * bip Customizer Notification Section Class.
 *
 * @package bip
 */

/**
 * bip_Customizer_Notice_Section class
 */
class bip_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $bip_recommended_plugins as $slug => $plugin_opt ) {
			...
			$bip_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$bip_recommended_plugin['class'] = 'active';
			} else {
				$bip_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$bip_recommended_plugin['button_class'] = 'install-now button';
					$bip_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$bip_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$bip_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$bip_recommended_plugin['button_class'] = 'deactivate-now button';
					$bip_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$bip_recommended_plugin['id'] = $slug;
			$bip_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$bip_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$bip_recommended_plugin['description'] = $info->short_description;
			}

			$bip_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $bip_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="bip-recommeded-actions-container bip-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="bip-recommeded-actions">
