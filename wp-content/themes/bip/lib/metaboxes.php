		...
		public function __construct() {
			$this->fields	 = array(
				array(
					...
					'label'	 => esc_html__( 'Transparent header', 'bip' ),
					'type'	 => 'checkbox',
				),
				array(
					'id'	 => 'header-font-color',
					'label'	 => esc_html__( 'Transparent header font color', 'bip' ),
					'type'	 => 'color',
				),
			);
			...
			if ( !empty( $post ) ) {
				...
				if ( $pageTemplate == 'template-parts/template-homepage.php' ) {
					foreach ( $this->screens as $screen ) {
						add_meta_box(
						'header-options', esc_html__( 'Header Options', 'bip' ), array( $this, 'add_meta_box_callback' ), $screen, 'normal', 'high'
						);
					}
				}
			}
