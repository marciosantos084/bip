	...
	class awpqsfprocess{
	   ...
	   function ajax_result($arg, $id,$pagenumber){
		   $apiclass = new ajaxwpqsfclass();
			$url_arquivo = get_field('link_do_arquivo');	
		  ...
		  // The Query
			...
			// The Loop
		if ( $query->have_posts() ) {
			$html .= '<ul class="content-edicoes">';
			while ( $query->have_posts() ) {
				$query->the_post();
					$html .= '<li id="edicao-'. get_the_ID().'" class="loop-edicao"><div class="image-edicao"><a href="'. get_field('link_do_arquivo') .'" rel="bookmark" target="_blank">'.get_the_post_thumbnail( $post->ID, 'full' ).'</a></div>';
					$html .= '';
					$html .= '';
					$html .= '<div class="content"><a href="'. get_field('link_do_arquivo') .'"  data-bind="attr: { href: Url }" download="" ><img src="http://coletivocladestinos.com/bip/wp-content/uploads/2018/10/bullet-download.png" alt="" width="30" height="30" class="float-left size-full wp-image-23" /></a><div class="footer"><h4><a href="'. get_field('link_do_arquivo') .'" rel="bookmark" target="_blank">'.get_the_title().'</a></h4><div class="excerpt">'.get_the_excerpt().'</div></div></div></li>';
