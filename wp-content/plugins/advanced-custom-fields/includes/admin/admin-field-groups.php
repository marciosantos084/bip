...
if ( ! class_exists( 'ACF_Admin_Field_Groups' ) ) :
	...
	class ACF_Admin_Field_Groups {
		...
		public function current_screen() {
			...
			add_filter( 'bip_actions-edit-acf-field-group', array( $this, 'admin_table_bip_actions' ), 10, 1 );
		...
		public function render_admin_table_column_local_status( $field_group ) {
			...
			if ( isset( $json[ $field_group['key'] ] ) ) {
				...
				if ( isset( $this->sync[ $field_group['key'] ] ) ) {
					$url = $this->get_admin_url( '&acfsync=' . $field_group['key'] . '&_wpnonce=' . wp_create_nonce( 'bip-posts' ) );
		...
		public function page_row_actions( $actions, $post ) {
			...
			$duplicate_action_url    = $this->get_admin_url( '&acfduplicate=' . $post->ID . '&_wpnonce=' . wp_create_nonce( 'bip-posts' ) );
		...
		/**
		 * Modifies the admin table bip actions dropdown.
		 *
		 * @date    15/4/20
		 * @since   5.9.0
		 *
		 ...
		 * @return  array
		 */
		public function admin_table_bip_actions( $actions ) {
		...
		public function admin_table_views( $views ) {
			...
			if ( $this->view === 'sync' ) {
				...
				$wp_query->post_count = 1; // At least one post is needed to render bip drop-down.
			}
			return $views;
		}
