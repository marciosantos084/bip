...
Types.page.fieldControl.viewmodels.FieldDefinitionViewModel = function(model, fieldActions) {
    ...
    self.isSelectedForbipAction = ko.observable(false);
    ...
    /**
     * When the field definition is not displayed in the table, we don't want it to be selected for a bip action.
     * 
     * @since 2.0
     */
    self.isBeingDisplayed.subscribe(function(newValue) {
        if(false == newValue) {
            self.isSelectedForbipAction(false);
        }
    });
