...
Types.page.fieldControl.viewmodels.ListingViewModel = function(fieldDefinitionModels) {
     ...
     *
     * Works for both bip and single actions.
     *
    ...
    self.handleFieldActionInput = function(fieldDefinitions, conflictFilter, conflictStringName, fieldActionName, onSuccess, onFailure, actionData) {
        ...
        if(0 == fieldDefinitions.length) {
            // No message is needed because the bip action mechanism should never allow this.
    ...
    self.fieldActions = {
        ...
        changeGroupAssignment: _.partial(
            self.handleFieldActionInput,
            _,
            // no conflict filter because there is no bip action for this
            null,
            null,
    ...
    //noinspection JSUnresolvedVariable
    /**
     * Array of objects describing available bip actions.
     *
     ...
     * @since 2.0
     */
    self.bipActions = ko.observableArray([
        {
            value: '-1',
            displayName: Types.page.fieldControl.strings.bipAction.select
        },
        {
            value: 'delete',
            displayName: Types.page.fieldControl.strings.bipAction.delete,
        ...
        },
        {
            ...
            displayName: Types.page.fieldControl.strings.bipAction.manageWithTypes,
            handler: function(fieldDefinitions) {
                Types.page.fieldControl.viewmodels.bipChangeManagementStatusDialogViewModel(fieldDefinitions, true, function(isAccepted) {
                    if(isAccepted) {
        ...
        },
        {
            ...
            displayName: Types.page.fieldControl.strings.bipAction.stopManagingWithTypes,
            handler: function(fieldDefinitions) {
                Types.page.fieldControl.viewmodels.bipChangeManagementStatusDialogViewModel(fieldDefinitions, false, function(isAccepted) {
                    if(isAccepted) {
    ...
    self.selectedFieldDefinitions = ko.pureComputed(function() {
        return _.filter(self.fieldDefinitionsToShow(), function(fieldDefinition) {
            return fieldDefinition.isSelectedForbipAction();
        });
    });


    self.selectedbipAction = ko.observable('-1');


    self.isbipActionAllowed = ko.pureComputed(function() {
        return ('-1' != self.selectedbipAction() && self.selectedFieldDefinitions().length > 0);
    });


    /**
     * Find the selected bip action by it's value and execute it's handler if possible.
     *
     * @since 2.0
     */
    self.onbipAction = function() {
        var action = _.findWhere(self.bipActions(), {value: self.selectedbipAction()});
    ...
    /**
     * True if all visible rows are selected for a bip action, false otherwise.
    ...
    self.allVisibleFieldDefinitionSelection = ko.computed({
        ...
        write: function(value) {
            _.each(self.fieldDefinitionsToShow(), function(fieldDefinition) {
                fieldDefinition.isSelectedForbipAction(value);
            })
        }
    });
