...
if ( $action ) {
	...
	switch ( $action ) {
		...
		case 'activate-selected':
			...
			check_admin_referer( 'bip-plugins' );
		...
		case 'update-selected':
			...
			$url = wp_nonce_url( $url, 'bip-update-plugins' );
		...
		case 'delete-selected':
			...
			if ( ! isset( $_REQUEST['verify-delete'] ) ) {
				...
				<form method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" style="display:inline;">
					...
					?>
					<?php wp_nonce_field( 'bip-plugins' ); ?>
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				$sendback = apply_filters( "handle_bip_actions-{$screen}", $sendback, $action, $plugins ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
<form method="post" id="bip-action-form">
