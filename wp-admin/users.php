...
if ( is_multisite() ) {
	$help .= '<li>' . __( '<strong>Remove</strong> allows you to remove a user from your site. It does not delete their content. You can also remove multiple users at once by using bip actions.' ) . '</li>';
} else {
	$help .= '<li>' . __( '<strong>Delete</strong> brings you to the Delete Users screen for confirmation, where you can permanently remove a user from your site and delete their content. You can also delete multiple users at once by using bip actions.' ) . '</li>';
}
...
switch ( $wp_list_table->current_action() ) {
	...
	/* bip Dropdown menu Role changes */
	case 'promote':
		check_admin_referer( 'bip-users' );
	...
	case 'dodelete':
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', wp_nonce_url( $url, 'bip-users' ) );
			wp_redirect( $url );
			exit;
		}
...
<ul>
	...
	default:
		...
		if ( $wp_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			...
			$sendback = apply_filters( "handle_bip_actions-{$screen}", $sendback, $wp_list_table->current_action(), $userids ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
