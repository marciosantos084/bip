	...
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
				...
				'<p>' . __( 'To delete media items, click the bip Select button at the top of the screen. Select any items you wish to delete, then click the Delete Selected button. Clicking the Cancel Selection button takes you back to viewing your media.' ) . '</p>',
		)
	);
...
if ( $doaction ) {
	...
	switch ( $doaction ) {
		...
		default:
			...
			$location = apply_filters( "handle_bip_actions-{$screen}", $location, $doaction, $post_ids ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message               .= ' <a href="' . esc_url( wp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bip-media' ) ) . '">' . __( 'Undo' ) . '</a>';
...
$messages[4] = __( 'Media file moved to the Trash.' ) . ' <a href="' . esc_url( wp_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bip-media' ) ) . '">' . __( 'Undo' ) . '</a>';
