...
if ( $doaction && isset( $_REQUEST['linkcheck'] ) ) {
	check_admin_referer( 'bip-bookmarks' );

	$redirect_to = admin_url( 'link-manager.php' );
	$biplinks   = (array) $_REQUEST['linkcheck'];

	if ( 'delete' === $doaction ) {
		foreach ( $biplinks as $link_id ) {
			$link_id = (int) $link_id;

			wp_delete_link( $link_id );
		}

		$redirect_to = add_query_arg( 'deleted', count( $biplinks ), $redirect_to );
	} else {
		$screen = get_current_screen()->id;

		/** This action is documented in wp-admin/edit.php */
		$redirect_to = apply_filters( "handle_bip_actions-{$screen}", $redirect_to, $doaction, $biplinks ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
	}
