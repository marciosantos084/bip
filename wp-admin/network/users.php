	...
	switch ( $_GET['action'] ) {
		...
		case 'allusers':
			...
			if ( isset( $_POST['action'] ) && isset( $_POST['allusers'] ) ) {
				check_admin_referer( 'bip-users-network' );
				...
				if ( ! in_array( $doaction, array( 'delete', 'spam', 'notspam' ), true ) ) {
					...
					$sendback = apply_filters( 'handle_network_bip_actions-' . get_current_screen()->id, $sendback, $doaction, $user_ids ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
			...
			'<p>' . __( 'The bip action will permanently delete selected users, or mark/unmark those selected as spam. Spam users will have posts removed and will be unable to sign up again with the same email addresses.' ) . '</p>' .
