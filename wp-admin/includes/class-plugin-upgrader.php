...
class Plugin_Upgrader extends WP_Upgrader {
	...
	/**
	 * Whether a bip upgrade/installation is being performed.
	 *
	 * @since 2.9.0
	 * @var bool $bip
	 */
	...
	public function upgrade_strings() {
		...
		$this->strings['process_bip_success'] = __( 'Plugins updated successfully.' );
	}
	...
	/**
	 * bip upgrade several plugins at once.
	 *
	 * @since 2.8.0
	 * @since 3.7.0 The `$args` parameter was added, making clearing the plugin update cache optional.
	 *
	 ...
	 */
	public function bip_upgrade( $plugins, $args = array() ) {
		$defaults    = array(
			'clear_update_cache' => true,
		);
		$parsed_args = wp_parse_args( $args, $defaults );
	...
	public function active_before( $return, $plugin ) {
		...
		// Change to maintenance mode. bip edit handles this separately.
		if ( ! $this->bip ) {
			$this->maintenance_mode( true );
		}
	...
	public function active_after( $return, $plugin ) {
		...
		// Time to remove maintenance mode. bip edit handles this separately.
		if ( ! $this->bip ) {
			$this->maintenance_mode( false );
		}
	 ...
	 * Plugin_Upgrader::upgrade() and Plugin_Upgrader::bip_upgrade().
	 *
	 * @since 2.8.0
	 *
	 * @global WP_Filesystem_Base $wp_filesystem WordPress filesystem subclass.
