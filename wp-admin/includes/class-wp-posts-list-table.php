...
class WP_Posts_List_Table extends WP_List_Table {
	 ...
	 * @return array
	 */
	protected function get_bip_actions() {
		$actions       = array();
	...
	public function column_cb( $item ) {
		...
		/**
		 * Filters whether to show the bip edit checkbox for a post in its list table.
		 *
	...
	public function inline_edit() {
		...
		$bip_edit_classes   = "bip-edit-row bip-edit-row-$hclass bip-edit-{$screen->post_type}";
		...
		while ( $bip < 2 ) :
			...
			$classes .= $bip ? $bip_edit_classes : $quick_edit_classes;
			?>
			<tr id="<?php echo $bip ? 'bip-edit' : 'inline-edit'; ?>" class="<?php echo $classes; ?>" style="display: none">
			<td colspan="<?php echo $this->get_column_count(); ?>" class="colspanchange">

			<fieldset class="inline-edit-col-left">
				<legend class="inline-edit-legend"><?php echo $bip ? __( 'bip Edit' ) : __( 'Quick Edit' ); ?></legend>
				...
				if ( post_type_supports( $screen->post_type, 'author' ) ) {
					...
					if ( current_user_can( $post_type_object->cap->edit_others_posts ) ) {
						 ...
						 * @param bool  $bip      A flag to denote if it's a bip action.
						 */
						$users_opt = apply_filters( 'quick_edit_dropdown_authors_args', $users_opt, $bip );
			...
			<?php if ( count( $hierarchical_taxonomies ) && ! $bip ) : ?>
			...
			<?php endif; // count( $hierarchical_taxonomies ) && ! $bip ?>

			<fieldset class="inline-edit-col-right">
				<div class="inline-edit-col">

				<?php
				if ( post_type_supports( $screen->post_type, 'author' ) && $bip ) {
					echo $authors_dropdown;
				}
				?>
				...
				<?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?>
					...
					<?php if ( $post_type_object->hierarchical ) : ?>
						...
						<label>
							 ...
							 *
							 * @since 2.7.0
							 * @since 5.6.0 The `$bip` parameter was added.
							 *
							 ...
							 * @param bool  $bip          A flag to denote if it's a bip action.
							 */
							$dropdown_args = apply_filters( 'quick_edit_dropdown_pages_args', $dropdown_args, $bip );
				...
				<?php if ( count( $flat_taxonomies ) && ! $bip ) : ?>
				...
				<?php endif; // count( $flat_taxonomies ) && ! $bip ?>
				...
				<?php if ( $bip && current_theme_supports( 'post-formats' ) && post_type_supports( $screen->post_type, 'post-formats' ) ) : ?>
			...
			foreach ( $columns as $column_name => $column_display_name ) {
				...
				if ( $bip ) {
					...
					/**
					 * Fires once for each column in bip Edit mode.
					 *
					 * @since 2.7.0
					 *
					 ...
					 * @param string $post_type   The post type slug.
					 */
					do_action( 'bip_edit_custom_box', $column_name, $screen->post_type );
				} else {
			...
			<div class="submit inline-edit-save">
				...
				<?php else : ?>
					<?php submit_button( __( 'Update' ), 'primary alignright', 'bip_edit', false ); ?>
				<?php endif; ?>

				<input type="hidden" name="post_view" value="<?php echo esc_attr( $m ); ?>" />
				<input type="hidden" name="screen" value="<?php echo esc_attr( $screen->id ); ?>" />
				<?php if ( ! $bip && ! post_type_supports( $screen->post_type, 'author' ) ) : ?>
