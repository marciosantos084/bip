...
class WP_Users_List_Table extends WP_List_Table {
	...
	/**
	 * Retrieve an associative array of bip actions available on this table.
	 *
	 * @since 3.1.0
	 *
	 * @return array Array of bip action labels keyed by their action.
	 */
	protected function get_bip_actions() {
		...
		// Add a password reset link to the bip actions dropdown.
	...
	/**
	 * Output the controls to allow user roles to be changed in bip.
	 *
	 * @since 3.1.0
	 *
	...
	<div class="alignleft actions">
		...
		/**
		 * Fires just before the closing div containing the bip role-change controls
	...
	/**
	 * Capture the bip action required, and return it.
	 *
	 ...
	 * @since 3.1.0
	 *
	 * @return string The bip action required.
	 */
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			if ( ! is_multisite() && get_current_user_id() != $user_object->ID && current_user_can( 'delete_user', $user_object->ID ) ) {
				$actions['delete'] = "<a class='submitdelete' href='" . wp_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bip-users' ) . "'>" . __( 'Delete' ) . '</a>';
			}
			if ( is_multisite() && current_user_can( 'remove_user', $user_object->ID ) ) {
				$actions['remove'] = "<a class='submitdelete' href='" . wp_nonce_url( $url . "action=remove&amp;user=$user_object->ID", 'bip-users' ) . "'>" . __( 'Remove' ) . '</a>';
			}
			...
			if ( get_current_user_id() !== $user_object->ID && current_user_can( 'edit_user', $user_object->ID ) ) {
				$actions['resetpassword'] = "<a class='resetpassword' href='" . wp_nonce_url( "users.php?action=resetpassword&amp;users=$user_object->ID", 'bip-users' ) . "'>" . __( 'Send password reset' ) . '</a>';
			}
