...
class WP_Plugins_List_Table extends WP_List_Table {
	 ...
	 * @param string $which
	 */
	public function bip_actions( $which = '' ) {
		global $status;
				...
				if ( $is_active ) {
					...
					if ( current_user_can( 'delete_plugins' ) && ! is_plugin_active( $plugin_file ) ) {
						$actions['delete'] = sprintf(
							...
							wp_nonce_url( 'plugins.php?action=delete-selected&amp;checked[]=' . urlencode( $plugin_file ) . '&amp;plugin_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bip-plugins' ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
