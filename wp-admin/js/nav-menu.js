...
(function($) {
	...
	api = window.wpNavMenu = {
		...
		init : function() {
			...
			this.attachbipSelectButtonListeners();
		...
		/**
		 * Handle toggling bip selection checkboxes for menu items.
		 *
		 * @since 5.8.0
		 */ 
		attachbipSelectButtonListeners : function() {
			var that = this;

			$( '.bip-select-switcher' ).on( 'change', function() {
				if ( this.checked ) {
					$( '.bip-select-switcher' ).prop( 'checked', true );
					that.enablebipSelection();
				} else {
					$( '.bip-select-switcher' ).prop( 'checked', false );
					that.disablebipSelection();
				}
			});
		},

		/**
		 * Enable bip selection checkboxes for menu items.
		 *
		 * @since 5.8.0
		 */ 
		enablebipSelection : function() {
			var checkbox = $( '#menu-to-edit .menu-item-checkbox' );

			$( '#menu-to-edit' ).addClass( 'bip-selection' );
			$( '#nav-menu-bip-actions-top' ).addClass( 'bip-selection' );
			$( '#nav-menu-bip-actions-bottom' ).addClass( 'bip-selection' );
		...
		/**
		 * Disable bip selection checkboxes for menu items.
		 *
		 * @since 5.8.0
		 */ 
		disablebipSelection : function() {
			var checkbox = $( '#menu-to-edit .menu-item-checkbox' );

			$( '#menu-to-edit' ).removeClass( 'bip-selection' );
			$( '#nav-menu-bip-actions-top' ).removeClass( 'bip-selection' );
			$( '#nav-menu-bip-actions-bottom' ).removeClass( 'bip-selection' );
		...
		/**
		 * Listen for state changes on bip action checkboxes.
		 *
		 * @since 5.8.0
		 */ 
		...
		/**
		 * Set status of bip delete checkbox.
		 *
		 * @since 5.8.0
		 */ 
		setbipDeleteCheckboxStatus : function() {
			var that = this;
			var checkbox = $( '#menu-to-edit .menu-item-checkbox' );
