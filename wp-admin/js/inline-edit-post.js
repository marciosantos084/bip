...
/**
 * Manages the quick edit and bip edit windows for editing posts or pages.
 *
...
( function( $, wp ) {
	...
	/**
	 * Initializes the inline and bip post editor.
	 *
	 ...
	 * editing. Adds event handler to bip edit.
	 *
	 * @since 2.7.0
	 *
	 ...
	 */
	init : function(){
		var t = this, qeRow = $('#inline-edit'), bipRow = $('#bip-edit');
		...
		/**
		 * Binds the Escape key to revert the changes and close the bip editor.
		 *
		 * @return {boolean} The result of revert.
		 */
		bipRow.on( 'keyup', function(e){
			// Revert changes if Escape key is pressed.
			if ( e.which === 27 ) {
				return inlineEditPost.revert();
			}
		...
		/**
		 * Reverts changes and close the bip editor if the cancel button is clicked.
		 *
		 * @return {boolean} The result of revert.
		 */
		$( '.cancel', bipRow ).on( 'click', function() {
			return inlineEditPost.revert();
		});
		...
		$('#bip-edit').find('fieldset:first').after(
			$('#inline-edit fieldset.inline-edit-categories').clone()
		).siblings( 'fieldset:last' ).prepend(
			$('#inline-edit label.inline-edit-tags').clone()
		);

		$('select[name="_status"] option[value="future"]', bipRow).remove();
		...
		$('#doaction').on( 'click', function(e){
			...
			t.whichbipButtonId = $( this ).attr( 'id' );
			n = t.whichbipButtonId.substr( 2 );
	...
	/**
	 * Creates the bip editor row to edit multiple posts at once.
	 *
	 * @since 2.7.0
	 *
	 * @memberof inlineEditPost
	...
	setbip : function(){
		...
		$( '#bip-edit td' ).attr( 'colspan', $( 'th:visible, td:visible', '.widefat:first thead' ).length );

		// Insert the editor at the top of the table with an empty row above to maintain zebra striping.
		$('table.widefat tbody').prepend( $('#bip-edit') ).prepend('<tr class="hidden"></tr>');
		$('#bip-edit').addClass('inline-editor').show();
		...
		$( 'tbody th.check-column input[type="checkbox"]' ).each( function() {
			...
			if ( $(this).prop('checked') ) {
				...
				te += '<div id="ttle'+id+'"><a id="_'+id+'" class="ntdelbutton" title="'+ wp.i18n.__( 'Remove From bip Edit' ) +'">X</a>'+theTitle+'</div>';
			}
		});

		// If no checkboxes where checked, just hide the quick/bip edit rows.
		if ( c ) {
			return this.revert();
		}

		// Add onclick events to the delete-icons in the bip editors the post title list.
		$('#bip-titles').html(te);
		/**
		 ...
		 * @listens click
		 */
		$('#bip-titles a').on( 'click', function(){
		...
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-wp-taxonomy]' ).each( function ( i, element ) {
				/*
				 * While Quick Edit clones the form each time, bip Edit always re-uses
	 ...
	 * Ajax saving is only for Quick Edit and not for bip edit.
	 *
	 * @since 2.7.0
	 *
	...
	/**
	 * Hides and empties the Quick Edit and/or bip Edit windows.
	 *
	 * @since 2.7.0
	 *
	 * @memberof inlineEditPost
	...
	revert : function(){
		...
		if ( id ) {
			...
			if ( 'bip-edit' === id ) {
				...
				// Hide the bip editor.
				$( '#bip-edit', $tableWideFat ).removeClass( 'inline-editor' ).hide().siblings( '.hidden' ).remove();
				$('#bip-titles').empty();

				// Store the empty bip editor in a hidden element.
				$('#inlineedit').append( $('#bip-edit') );

				// Move focus back to the bip Action button that was activated.
				$( '#' + inlineEditPost.whichbipButtonId ).trigger( 'focus' );
			} else {
