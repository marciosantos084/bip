...
window.setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		...
		if ( action ) {
			...
			el.find('.check-column :checkbox').prop('checked', false); // Uncheck the row so as not to be affected by bip Edits.
...
$( function(){
	var make_hotkeys_redirect, edit_comment, toggle_all, make_bip;
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		/**
		 * Toggles all comments on the screen, for bip actions.
		 *
		 * @since 2.7.0
		 * @access private
		 *
		...
		/**
		 * Creates a bip action function that is executed on all selected comments.
		 *
		 * @since 2.7.0
		 * @access private
		 *
		 ...
		 *
		 * @return {Function} The function that executes the bip action.
		 */
		make_bip = function(value) {
			return function() {
		...
		$.table_hotkeys(
			...
			[
				...
				['e', edit_comment],
				['shift+x', toggle_all],
				['shift+a', make_bip('approve')],
				['shift+s', make_bip('spam')],
				['shift+d', make_bip('delete')],
				['shift+t', make_bip('trash')],
				['shift+z', make_bip('untrash')],
				['shift+u', make_bip('unapprove')]
			],
			{
