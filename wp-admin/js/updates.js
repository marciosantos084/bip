...
(function( $, wp, settings ) {
		...
		if ( ! $pluginUpdateRow.length ) {
			$plugin.addClass( 'update' ).after(
				pluginUpdateRow( {
					...
					colspan: $( '#bip-action-form' ).find( 'thead th:not(.hidden), thead td' ).length,
	...
	$( function() {
		 ...
		 * @param {Event} event Event interface.
		 */
		$bipActionForm.on( 'click', '[data-plugin] .update-link', function( event ) {
		 ...
		 * @param {Event} event Event interface.
		 */
		$bipActionForm.on( 'click', '[data-plugin] a.delete', function( event ) {
		...
		/**
		 * bip action handler for plugins and themes.
		 *
		 ...
		 * @param {Event} event Event interface.
		 */
		$bipActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			var bipAction    = $( event.target ).siblings( 'select' ).val(),
				itemsSelected = $bipActionForm.find( 'input[name="checked[]"]:checked' ),
				success       = 0,
				error         = 0,
				errorMessages = [],
				type, action;
			...
			switch ( bipAction ) {
				case 'update-selected':
					action = bipAction.replace( 'selected', type );
					break;
			...
			// Un-check the bip checkboxes.
			$bipActionForm.find( '.manage-column [type="checkbox"]' ).prop( 'checked', false );

			$document.trigger( 'wp-' + type + '-bip-' + bipAction, itemsSelected );
			...
			itemsSelected.each( function( index, element ) {
				...
				if ( 'update-selected' === bipAction && ( ! $itemRow.hasClass( 'update' ) || $itemRow.find( 'notice-error' ).length ) ) {
			...
			// Display bip notification for updates of any kind.
			$document.on( 'wp-plugin-update-success wp-plugin-update-error wp-theme-update-success wp-theme-update-error', function( event, response ) {
				...
				wp.updates.adminNotice = wp.template( 'wp-bip-updates-admin-notice' );

				wp.updates.addAdminNotice( {
					id:            'bip-action-notice',
					className:     'bip-action-notice',
					successes:     success,
					errors:        error,
				...
				$bipActionNotice = $( '#bip-action-notice' ).on( 'click', 'button', function() {
					// $( this ) is the clicked button, no need to get it again.
					$( this )
						.toggleClass( 'bip-action-errors-collapsed' )
						.attr( 'aria-expanded', ! $( this ).hasClass( 'bip-action-errors-collapsed' ) );
					// Show the errors list.
					$bipActionNotice.find( '.bip-action-errors' ).toggleClass( 'hidden' );
				} );
			...
			// Reset admin notice template after #bip-action-notice was added.
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			wp.updates.searchRequest = wp.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				$( 'body' ).removeClass( 'loading-content' );
				$bipActionForm.append( response.items );
				delete wp.updates.searchRequest;
